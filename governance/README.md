...
The independent [Java source journey](java-ci-journey/README.md) closes that gap: its CI job builds the pinned Scanner `0.2.1` commit, scans baseline → introduced → repaired synthetic revisions and verifies the exact reports and governance classifications. It does not replace the previously published Scanner `0.2.0` golden reports.
